...
Today I'm interviewing Rainer Pope, who is CEO of Maddox, which is a new chip startup. Previously, he was doing TPU architecture and many other things at Google. This is a very different format from my usual interviews. This is going to be a Blackboard lecture. We're going to get up in a second.

0:13

We, in fact, built this whole new studio with specifically this format in mind. And so it's a pleasure to get to inaugurate it with you. We're going to be talking about model architecture, ML Infra, many other things. And the reason I think it's an important topic is

0:27

is because once you actually understand how training and inference actually work in a cluster, as we'll see a lot of things about why AI is the way it is, why AI architectures are the way they are, why API prices are the way they are, fundamentally also how why AI progress is the way it is, start making sense.

0:45

And you need to understand the details to get there and you need a blackboard to understand the details. So Reiner, thank you so much for doing this. Yeah, very happy to be here. Just a heads up, this is a lecture with graphs and equations and all that stuff.

0:56

So if you can, I would really recommend watching it on a video platform like YouTube. Okay, full disclosure, I am an angel investor in Maddox, but that's unrelated to this podcast. Reiner, maybe to kick us off, I'll ask this question. So we have a couple of companies like Claude and Codex and Cursor are offering something like FastMode,

## Reiner Pope – The math behind how LLMs are trained and servedReiner Pope – LLMs 训练和服务背后的数学

It's shocking how much you can deduce about what the labs are doing from a handful of equations and a blackboard  
从几个方程和黑板上可以推断出实验室在做什么，这真是令人震惊
...
与 Reiner Pope 进行了一次非常不同的格式——他在黑板上讲解了前沿大模型的训练和服务方式。
...
世界上理解从芯片设计到模型架构的整个 AI 栈的人寥寥无几，Reiner 就是其中之一。从他那里学习真是一大乐事。
...
写了一些闪卡和练习题来帮助自己记住 Reiner 所教的内容。希望对你也有帮助！
...
- [Jane Street](https://janestreet.com/dwarkesh) needs constant access to incredibly low-latency compute. I recently asked one of their engineers, Clark, to talk me through how they meet these demands. Our conversation—which touched on everything from FPGAs to liquid cooling—was extremely helpful as I prepped to interview Reiner. You can watch the full discussion and explore Jane Street’s open roles at [janestreet.com/dwarkesh](https://janestreet.com/dwarkesh)  
	简街需要持续访问极低延迟的计算资源。我最近请教了他们的一位工程师克拉克，让他向我解释他们是如何满足这些需求的。我们的对话——从 FPGA 到液体冷却——对我准备采访德雷克斯非常有帮助。你可以在 janestreet.com/dwarkesh 观看完整的讨论并探索简街的开放职位。
- [Google’s Gemma 4](https://goo.gle/Gemma4) is the first open model that’s let me shut off the internet and create a fully disconnected “focus machine”. This is because Gemma is small enough to run on my laptop, but powerful enough to actually be useful. So, to prep for this interview, I downloaded Reiner’s scaling book, disconnected from wifi, and used Gemma to help me break down the material. Check it out at [goo.gle/Gemma4](https://goo.gle/Gemma4)  
	谷歌的 Gemma 4 是我使用过的首个开放模型，它让我能够断开互联网并创建一台完全离线的“专注机器”。这是因为 Gemma 足够小巧，可以在我的笔记本电脑上运行，但又足够强大，实际使用中非常有用。因此，为了准备这次采访，我下载了 Reiner 的扩展书籍，断开 WiFi，并使用 Gemma 帮助我分解材料。请在 goo.gle/Gemma4 查看。
...
### 00:00:00 – How batch size affects token cost and speed00:00:00 – 批量大小如何影响令牌成本和速度
...
今天，我正在采访 Reiner Pope，他是 MatX 的首席执行官，MatX 是一家新成立的芯片初创公司。此前，他在谷歌从事 TPU 架构以及许多其他工作。这与我通常的采访形式非常不同。这将是一场黑板讲座。我们马上就会开始。事实上，我们专门为这种形式建造了整个全新的工作室，所以很高兴能与您一起 inaugurate 它。
...
我们将讨论模型架构、ML 基础设施以及其他许多内容。我认为这是一个重要话题的原因在于，一旦你理解了集群中训练和推理的工作原理，很多事情——关于为什么 AI 是现在这个样子，为什么 AI 架构是现在这个样子，为什么 API 价格是现在这个样子，以及从根本上说为什么 AI 进展是现在这个样子——就开始变得有意义了。你需要理解细节才能达到这个水平，而你需要一块黑板来理解这些细节。Reiner，非常感谢你做这个。
...
完全透明，我在 MatX 担任天使投资人，但这与这个播客无关。Reiner，为了开始我们的讨论，我会问这个问题。我们有一些公司，比如 Claude 和 Codex 以及 Cursor，它们提供类似 Fast Mode 的服务，即以 6 倍的价格，以 2.5 倍的速度为你提供代币流。从技术层面上，我很好奇这里发生了什么。为什么支付更多就可以获得更快的延迟？
...
二，你能继续吗？你能支付 100 倍的费用并设法获得更快的速度吗？三，你能换个方向吗？你能有类似克劳德代码“慢速模式”的东西吗？如果你愿意等待几分钟，你能得到更便宜的价格吗？也许这将有助于激励你通过讲座进行分析。
...
![[ac699de1cb1055d6f07308c308f2dd8a_MD5.webp]]
...
This is the total memory requirement across the system, but now I’m going to calculate a memory requirement per GPU. I’ll use a lowercase *c* <sub>mem</sub>. Obviously, we just take all of these numbers and divide it by *E* and P. Really easy. It’s this *N* <sub>total</sub>, plus the batch times length of context times bytes per token, all divided by *E* times *P*.
...
For many curves, the minimum tends to be where the costs are equalized. That’s something of a heuristic claim, but there are many examples where it’s true. Where one is 1/ *x* and the other one is *x*, for example, they tend to be minimized at the point where they equal each other. It’s also true for e *<sup>x</sup>* and e *<sup>-x</sup>* and all kinds of other things. Basically, I’ve got some curve that’s going down, some other curve that’s going up, and they tend to be minimized at this equal point.
...
I think often the number of active parameters could be in the range of a hundred billion, something like that. Maybe a bit larger. So multiply by 20 to get the Chinchilla token count. So Chinchilla, *D <sub>Chinchilla</sub>*, would be around two trillion. We see we’re about a hundred times larger than that.

**Dwarkesh Patel**

What does *D <sub>Chinchilla</sub>* actually mean?
...
To get this to kilobytes, you could get that, for example, as a *d* <sub>head</sub> of 128, which is typical. Then the number of bytes is typically the number of attention layers times two times *d* <sub>head</sub> times the number of KV heads. This is the number of unique contexts per layer.
...
Oh, okay. I see. That makes sense. Getting back to it. So *t* <sub>compute</sub>, if you have basically just this divided by len-pass, so just this amount. This actually does not vary based on *t*, so it’ll just be some flat value like this. And this is *t* <sub>compute</sub>. And this is—
...
Decode. Right. Now *t* <sub>mem</sub>, we have this whole thing divided by len-pass. Well, it doesn’t really matter what’s up there, it’ll just be something that looks like this. Let’s say this is *t* <sub>mem</sub>. This is decode again.
