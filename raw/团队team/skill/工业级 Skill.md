---
ingested: 2026-05-06
wiki_page: "[[wiki/sources/Source - 如何写出工业级 Skill]]"
title: "如何写出工业级 Skill"
source: "https://x.com/fakemaidenmaker/status/2051111166416396713?s=46"
author:
  - "[[@fakemaidenmaker]]"
published: 2026-05-04
created: 2026-05-04
description: "很多人自己写 Skill，会把背景、规则、注意事项、示例，全都塞进一个 Skill 文件。看起来很完整，但实际并不好用。Skill 真正的价值在于：当用户提出某类需求时，Agent 能自动识别场景，加载对应流程，使用合适工具，并按固定方法完成任务。并且保证以后遇到类似任务，Age..."
tags:
  - "clippings"
---
...
再比如一个“生成 X 长文”的 Skill，用户不一定会说“调用 article skill”。
...
还有一种情况：如果你有很多 Skill，或者某个 Skill 本身很长，可以在 Skill 里设置 disable，关闭自动加载，降级为 slash command。
...
SKILL.md 不应该承载所有内容，内容太长会让 Context 被瞬间占满，直接导致 ClaudeCode 性能变差，思考时间变长，甚至触发 Compact 导致失忆。
...
生成封面图/ SKILL.md references/ 封面图风格参考.md 常见平台尺寸.md 好坏案例对比.md scripts/ 检查图片尺寸.py 导出多平台版本.py assets/ 封面图模板.md 字体与配色示例.json
...
更合理的做法是：**让 Agent 根据 Skill 的用途，自动生成第一批 test data 和 eval cases**，你只需要检查这些测试是否贴近真实场景。
...
- 触发失败：在 description 中增加案例。
- 误触发：收窄 description，加入什么时候不触发的描述。
- 步骤漏掉：改 SKILL.md 的步骤内容。
- 输出格式不稳定：加输出样例到 assets/。
- 确定性检查不稳定：写 script。
...
写 Skill → 准备 test data → 运行 eval → 逐项打分 → 找失败原因 → 修改 Skill → 再跑 eval → 达到最低分数线，或者至少知道它的边界
...
- 哪些内容应该留在主文件，哪些应该拆出去？
- 我要怎么证明它真的有效？
- 如何不断维护迭代让这个 Skill 更好用？

这才是 Skill 和 Prompt 最大的区别。

Skill 并不只是单纯把提示词写长保存下来。
