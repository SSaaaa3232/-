---
ingested: 2026-05-06
wiki_page: "[[wiki/sources/Source - Claude Code CLI 拆解]]"
created: 2026-04-04
modified: 2026-04-04
tags:
  - AI
...
自动 compact 机制处理上下文窗口溢出。在每轮迭代开始时，估算当前 messages 数组的 token 数（总字符数除以 4），如果超过模型上下文限制的 85%，触发压缩——发起一次独立的 API 调用生成摘要，用 [{role: 'user', content: summary}, {role: 'assistant', content: 'Understood.'}] 替换原来的 messages 数组。（这点跟 Claude Code 是一样的）

Prompt Caching 在 Agent Loop 层有三个施力点：

*   system prompt blocks — 如前文所述，静态段走缓存
*   tools 数组 — 最后一个 tool definition 打上 cache_control
...
工具系统的入口是 TOOL_DEFINITIONS，一个 JSON Schema 数组，描述每个工具的名称、用途和参数结构。模型通过这个数组"知道"有哪些工具可以调用以及如何调用。数组中包含 21 个内置工具定义，加上运行时从 MCP 服务器动态注入的工具。（json scheme能保证输出格式，LLM json也更易读）

执行入口是单一的 executeTool 函数，内部用 switch 按工具名分发。MCP 工具通过 mcp__ 前缀识别，走独立的调用路径。

核心工具的实现细节各有侧重：

*   Read — fs.readFile 读取文件后加行号前缀，支持 offset 和 limit 参数分页读取
*   Write — 写入前 fs.mkdir({ recursive: true }) 确保父目录存在
*   Edit — 精确字符串替换，关键约束是 old_string 必须在文件中唯一出现，多次出现则报错——迫使模型提供足够精确的定位字符串
...
两阶段分类器增强 auto 模式下的安全性：

Stage 1 — 纯模式匹配：维护已知安全命令和已知危险命令的规则表，命中即返回，覆盖 90% 以上的情况，零延迟。

Stage 2 — 处理未覆盖的情况：将命令字符串发给 Haiku 模型，要求返回 allow/deny/ask_user 三种之一。Haiku 延迟约 300-500ms，相比模型主循环几乎可以忽略。ask_user 路径让自动模式在遇到模糊操作时能够优雅降级。

MCP（Model Context Protocol）本质上是一个标准化的工具发现协议。传统做法是把工具硬编码在 CLI 里，MCP 让工具变成可以独立部署的进程，通过统一接口被任何兼容客户端发现和调用。

协议层是 JSON-RPC 2.0 over stdio：启动 MCP server 进程，通过 stdin/stdout 交换 JSON-RPC 消息。启动序列固定：initialize 握手 → tools/list 获取工具定义数组。工具定义包含 JSON Schema 格式的 inputSchema，与 Anthropic API 的工具格式直接兼容。McpManager 管理多个 server 的生命周期，工具名加上 mcp server_name 前缀做命名空间隔离。

LSP 集成解决不同的问题：不是扩展工具，而是给模型提供实时的代码诊断信息。LspClient 实现 Language Server Protocol 客户端侧，使用 Content-Length 帧协议通信。LspManager 维护文件扩展名到语言服务器的路由表，Write/Edit 执行后自动通知对应语言服务器，诊断结果作为 lsp_diagnostics section 注入下一轮 system prompt。
...
---
标签: #AI-CLI #Claude-Code #Agent #Harness-Engineering
