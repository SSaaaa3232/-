...
aliases:
---
**命令行日志分析脚本**，主要用来分析 `modelbox.jsonl` 里的某次请求，估算 prompt 各部分“占了多少 token”。

**它在做什么**
- 读取 JSONL 日志（默认 `logs/modelbox.jsonl`）。
- 选中一条 `direction==="request"` 且 `body.input` 是数组的请求记录。
- 支持按 `--traceId` 精确选记录，或按 `--index` 选第几条（默认 `-1` 表示最新一条）。
- 提取 `system` prompt 文本并拆分成多个 section（总量、Project Context 各文件块、tail 区块等）。
- 对每个 section 统计字符数，并用“字符数换算”粗估 token（`chars/4 ~ chars/2.5`，取中间值为 est）。
- 统计 `body.tools` 的体积（JSON 压缩后字符/token 估算）。
- 比较 system 里 `## Tooling` 的工具名与 `body.tools` 的工具名重叠情况。
- 输出文本报告或 JSON 报告。

**你可以这样用**
- 默认分析最新请求：`node src/analyze-log.mjs`
- 指定日志文件：`node src/analyze-log.mjs --file path/to/file.jsonl`
- 按索引：`node src/analyze-log.mjs --index 0`
- 按 traceId：`node src/analyze-log.mjs --traceId abc123`
- 输出 JSON：`node src/analyze-log.mjs --json`

**一句话总结**
它不是业务逻辑脚本，而是一个“**排查 prompt 体积/结构**”的诊断工具，帮助你看 system prompt、上下文文件注入、tools 声明谁最占 token。
