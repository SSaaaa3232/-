---
name: "source-command-canvas-skill"
...
# source-command-canvas-skill

Use this skill when the user asks to run the migrated source command `canvas-SKILL`.
...
### from banana (`/canvas from banana`) (if the banana-Codex plugin is installed)
...
## Banana Integration (if the banana-Codex plugin is installed)
...
presentations), see [Codex-canvas](https://github.com/AgriciDaniel/Codex-canvas).
This skill handles wiki-scoped visual boards. Codex-canvas handles full-featured
