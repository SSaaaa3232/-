...
The Paseo CLI 允许您从终端管理代理。它与守护进程的 API 公开的界面相同，因此您在应用程序中可以执行的任何操作都可以通过命令行执行。
...
> 代理编排：您可以指示编码代理使用 Paseo CLI 生成和管理其他代理。这使您能够实现多代理工作流程，其中一个代理将子任务委托给其他代理并等待结果。
...
`--worktree` 标志在隔离的 git 工作树中创建代理，适用于并行功能开发。
...
- `$PASEO_WORKTREE_PATH` — 工作树目录 `--json` — JSON output
